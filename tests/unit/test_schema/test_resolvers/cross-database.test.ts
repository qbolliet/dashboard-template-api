...
beforeAll(async () => {
    ...
    if (!result.errors) {
        availableDatabases = (result.data!.getDatabases as Array<{ id: string }>).map(d => d.id);
    }
}, 60000);
...
describeCrossDB('compareFacts (enabled)', () => {
    ...
    `;
        ...
        const deltas = (result.data!.compareFacts as { data: Array<{ delta: number | null }> })
            .data.map(d => d.delta).filter((d): d is number => d !== null);
