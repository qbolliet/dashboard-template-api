...
class CatalogMetadataLoader extends BaseQueryLoader {
    ...
    async loadAllMetadata(
        ...
        return rows.map(row => ({ ...row, is_categorical: Boolean(row.is_categorical) })) as CatalogMetadataRow[];
    }
}
...
const createCatalogMetadataLoader = () => {
    ...
    return loader.createLoader<string, CatalogMetadataRow[]>(
        (connection, catalogId) => loader.loadAllMetadata(connection, catalogId)
    );
};
...
const createCatalogDimensionNamesLoader = () => {
    ...
    return loader.createLoader<string, string[]>(
        (connection, catalogId) => loader.loadDimensionNames(connection, catalogId)
    );
};
