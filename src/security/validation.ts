 ...
 */
const validateInput = (input: unknown, rules: ValidationRule = ValidationRules.STRING): unknown => {
...
const validateString = (str: string, rules: StringValidationRule): string => {
  ...
  if (str.length < rules.minLength) {
    throw new GraphQLError(`String too short. Minimum length is ${rules.minLength}`, {
  ...
  if (str.length > rules.maxLength) {
    throw new GraphQLError(`String too long. Maximum length is ${rules.maxLength}`, {
